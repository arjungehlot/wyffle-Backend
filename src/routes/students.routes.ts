...
router.get("/profile", verifyToken, requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!uid) {
      return res.status(401).json({ success: false, error: "Unauthorized: UID missing" });
    }
...
router.put("/profile", verifyToken, requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    // Prevent sensitive fields from being updated by user
    ["uid", "status", "paymentStatus", "progressSteps", "createdAt"].forEach(
...
router.put("/:uid/status", verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    const validStatuses = ["shortlisted", "active", "completed", "rejected"];
...
router.put("/:uid/payment-status", verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    const validPaymentStatuses = ["paid", "pending", "failed", "not_selected"];
...
router.put("/:uid/progress", verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (typeof progressPercentage !== "number" || progressPercentage < 0 || progressPercentage > 100) {
      return res.status(400).json({ success: false, error: "Invalid progress percentage" });
    }
...
router.put("/:uid/progress-step", verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (!validSteps.includes(step)) {
      return res.status(400).json({ success: false, error: "Invalid progress step" });
    }

    await studentService.updateProgressStep(uid, step, completed);
    res.json({ success: true, message: "Progress step updated successfully" });
  } catch (error) {
    console.error("Error updating progress step:", error);
    res.status(500).json({ success: false, error: "Failed to update progress step" });
  }
});
